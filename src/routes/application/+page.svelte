<script>
	...
	const RESUME_FORM = 'https://google.com';
	...
	function getFormData() {
		const formData = {
			// PERSONAL
			full_name: document.getElementById('form-first-name').value,
			preferred_name: document.getElementById('form-last-name').value,
			...
			year_at_current_university: document.getElementById('form-year').value,
			is_transfer: document.getElementById('form-transfer').value === 'Yes',
			...
			// SHORT ANSWER
			prior_engineering_experience: document.getElementById('form-q1').value,
			why_ideahacks: document.getElementById('form-q2').value,
			hackathon_ideas: document.getElementById('form-q3').value,
			prior_hackathon_experience: document.getElementById('form-q5').value,
			suggested_parts: document.getElementById('form-q6').value,

			// MISCELLANEOUS
			needs_housing_assistance: document.getElementById('form-housing').value === 'Yes',
			shirt_size: document.getElementById('form-shirt-size').value,
			dietary_restrictions: document.getElementById('form-allergies').value
		};
	...
	function isValidFormData(formData) {
		let inputFieldsValid = {
			firstNameCompleted: !(formData.full_name === ''),
			lastNameCompleted: !(formData.preferred_name === ''),
			// emailCompleted: !(formData.email === ''),
			// phoneCompleted: !(formData.phone === ''),
			schoolCompleted: !(formData.school === ''),
			majorCompleted: !(formData.major === ''),
			yearCompleted: !(formData.year_at_current_university === ''),
			transferCompleted: !(formData.is_transfer === ''),
			hasTeamCompleted: !(formData.hasTeam === ''),
			q1Valid:
				!(formData.prior_engineering_experience === '') &&
				shortAnswerWordCounts['q1'] <= WORD_LIMITS.q1,
			q2Valid: !(formData.why_ideahacks === '') && shortAnswerWordCounts['q2'] <= WORD_LIMITS.q2,
			q3Valid: !(formData.hackathon_ideas === '') && shortAnswerWordCounts['q3'] <= WORD_LIMITS.q3,
			// q4Valid: !(formData.prior_hackathon_experience === ''),
		...
		inputFieldsFilled['form-transfer'] = inputFieldsValid.transferCompleted;
		...
		<form class="mx-auto w-full max-w-xl">
			...
			<div class="-mx-3 mb-6 flex flex-wrap">
				...
				<a
					target="_blank"
					href={RESUME_FORM}
					class="mx-3 rounded bg-orange-500 px-4 py-2 hover:bg-orange-700">Upload Resume</a
				>
			</div>
			...
			<div class="-mx-3 mb-6 flex flex-wrap">
				...
				<div class="w-full px-3">
					<label class="mb-2 block text-xs uppercase tracking-wide text-gray-700" for="form-q5">
						4. If you have prior hackathon experience, please share your experience with us. ({WORD_LIMITS.q5}
						Words)
					</label>
					<input
			...
			<div class="-mx-3 mb-6 flex flex-wrap">
				<div class="w-full px-3">
					<label class="mb-2 block text-xs uppercase tracking-wide text-gray-700" for="form-q6">
						5. What parts would you like to see at IDEA Hacks this year? ({WORD_LIMITS.q6}
						Words)
					</label>
					<textarea
