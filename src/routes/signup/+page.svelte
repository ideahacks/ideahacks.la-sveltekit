...
<div class="flex items-center justify-center min-h-screen" style="background: linear-gradient(0deg, #0C2F57 0%, #1B0029 100%);">
  <div class="bg-white/25 shadow-lg rounded-md p-8 w-[800px] h-[500px] flex">
    ...
    <div class="w-1/2 flex items-center justify-center overflow-hidden rounded-l-md">
      <img src="/src/lib/images/ideahacks2025.png" alt="signup illustration" class="w-full h-full object-cover" />
    </div>
    ...
    <div class="w-1/2 flex flex-col justify-center pl-8">
      ...
      <form on:submit|preventDefault={handleSignUp} class="space-y-4">
        ...
        <div>
          <label for="password" class="block text-sm font-medium mb-1 text-stone-100">Password</label>
          <input
            id="password"
            type="password"
            bind:value={password}
        ...
        <div>
          <label for="confirm" class="block text-sm font-medium mb-1 text-stone-100">Confirm password</label>
          <input
            id="confirm"
            type="password"
            bind:value={confirm}
