			...
			<div class="bg-white/10 backdrop-blur-md rounded-lg shadow-lg p-8">
				...
				<div class="space-y-6">
					...
					{#if data.status === 'accepted'}
						<div class="bg-white/5 rounded-lg p-6">
							<h2 class="text-2xl font-semibold text-white mb-2 font-mono">Team Status</h2>
									...
									{#if data.teamMembers.length > 0}
										<ul>
											{#each data.teamMembers as member}
												<li>
													{member.applications_2026?.name
														?? `${member.applications_2026?.first_name ?? ''} ${member.applications_2026?.last_name ?? ''}`.trim()
														?? member.uid}
												</li>
											{/each}
										</ul>
