				...
				<div class="space-y-6 mt-8">
					...
					<div class="bg-white/5 rounded-lg p-6 border border-white/10">
						...
						{#if data.pendingSentInvites.length === 0}
							<p class="text-white/70">No pending invites yet.</p>
						{:else}
							<ul class="space-y-2">
								{#each data.pendingSentInvites as invite}
									<li
										class="flex items-center justify-between rounded-lg border border-white/10 bg-black/20 px-4 py-3"
									>
										...
										<div class="flex items-center gap-3">
											...
											<form method="POST" action="?/deletePendingInvite">
												<input type="hidden" name="invite_id" value={invite.id} />
												<button
													type="submit"
													class="px-4 py-1 border border-red-300/40 text-red-200 rounded-lg
													transition-colors duration-200 font-mono uppercase tracking-wider
													hover:bg-red-500/20 hover:border-red-200/60"
												>
													Delete
												</button>
											</form>
